...
$link = LEGAL.'admin.php'.fusion_get_aidlink().'&section=cp';
...
if ((isset($_GET['action']) && $_GET['action'] == 'edit') && (isset($_GET['legal_id']) && isnum($_GET['legal_id']))) {
    $result = dbquery("SELECT * FROM ".DB_LEGAL." WHERE legal_id='".$_GET['legal_id']."' AND legal_type = 'cp'");
...
echo form_textarea('legal_text', $locale['lg_03'], $data['legal_text'], [
    'autosize'  => 1,
    'form_name' => 'inputform',
...
$result = dbquery("SELECT * FROM ".DB_LEGAL." WHERE legal_type = 'cp'");
