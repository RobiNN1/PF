...
class NewThread {
    public static function displayForumPostForm($info) {
        ...
        $tab_content .= '<div class="m-t-20 m-b-20">';
        ...
        if (!empty($info['attachment_field'])) {
            ...
            $tab_content .= '<div class="m-t-20 m-b-20">'.$info['attachment_field'].'</div>';
            $tab_content .= closetabbody();
        }
        ...
        echo !empty($info['last_posts_reply']) ? '<div class="card mt-4 mb-4 newthread">'.$info['last_posts_reply'].'</div>' : '';
        echo '</div>';
    }
}
