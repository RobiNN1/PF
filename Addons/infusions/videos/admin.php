...
function videos_listing() {
    ...
    echo '<div class="clearfix m-b-10">';
        echo '<span class="pull-right">'.sprintf($locale['vid_006'], $rows, $total_rows).'</span>';

        if (!empty($cat_opts) > 0 && $total_rows > 0) {
            echo '<div class="dropdown pull-left m-r-10">';
                echo '<a id="ddvfilter" class="btn btn-default btn-sm dropdown-toggle" style="width: 200px;" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
                ...
                echo '<ul class="dropdown-menu" aria-labelledby="ddvfilter" style="max-height: 180px; width: 200px; overflow-y: auto;">';
                    foreach ($cat_opts as $cat_id => $cat_name) {
                        $active = isset($_GET['filter_cid']) && $_GET['filter_cid'] == $cat_id;

                        echo '<li'.($active ? 'class="active"' : '').'>';
                            echo '<a class="text-smaller" href="'.clean_request('filter_cid='.$cat_id, ['section', 'rowstart', 'aid']).'">';
                                echo $cat_name;
                            echo '</a>';
                        echo '</li>';
                    }
    ...
    if ($rows > 0) {
            ...
            while ($data = dbarray($result)) {
                echo '<div class="col-xs-12 col-sm-4">';
                    ...
                    echo '<div class="overflow-hide">';
                        ...
                        echo '<div>';
                            echo $locale['vid_009'].' <a class="badge" href="'.FUSION_SELF.$aidlink.'&section=categories&action=edit&cat_id='.$data['video_cat_id'].'">'.$data['video_cat_name'].'</a>';
                            echo '<br/><span><i class="fa fa-clock-o"></i> '.$data['video_length'].'</span>';
                        echo '</div>';

                        echo '<div class="m-t-5">';
                            echo '<a class="m-r-10" href="'.FUSION_SELF.$aidlink.'&action=edit&section=form&video_id='.$data['video_id'].'">'.$locale['edit'].'</a>';
                            echo '<a class="m-r-10" href="'.FUSION_SELF.$aidlink.'&action=delete&section=form&video_id='.$data['video_id'].'">'.$locale['delete'].'</a>';
                        echo '</div>';
                    echo '</div>';
                    echo '</div></div>';
                echo '</div>';
        ...
        echo '<div class="well text-center">'.$locale['vid_008'].'</div>';
    }
}

$locale = fusion_get_locale('', VID_LOCALE);
add_to_title($locale['vid_title']);

add_breadcrumb(['link' => INFUSIONS.'videos/admin.php'.fusion_get_aidlink(), 'title' => $locale['vid_title']]);
...
if (!empty($_GET['section'])) {
    switch ($_GET['section']) {
        case 'form':
            add_breadcrumb(['link' => FUSION_REQUEST, 'title' => $edit ? $locale['edit'] : $locale['add']]);
            break;
        case 'categories':
            add_breadcrumb(['link' => FUSION_REQUEST, 'title' => $locale['vid_001']]);
            break;
        case 'submissions':
            add_breadcrumb(['link' => FUSION_REQUEST, 'title' => $locale['vid_002']]);
            break;
        case 'settings':
            add_breadcrumb(['link' => FUSION_REQUEST, 'title' => $locale['vid_003']]);
            break;
    }
}

opentable($locale['vid_title']);
...
$tab['title'][] = $locale['vid_title'];
$tab['id'][]    = 'list';
$tab['icon'][]  = 'fa fa-fw fa-play';
$tab['title'][] = $edit ? $locale['edit'] : $locale['add'];
$tab['id'][]    = 'form';
$tab['icon'][]  = 'fa fa-'.($edit ? 'pencil' : 'plus');
$tab['title'][] = $locale['vid_001'];
$tab['id'][]    = 'categories';
$tab['icon'][]  = 'fa fa-folder';
$tab['title'][] = $locale['vid_002'].'&nbsp;<span class="badge">'.dbcount("(submit_id)", DB_SUBMISSIONS, "submit_type='v'").'</span>';
$tab['id'][]    = 'submissions';
$tab['icon'][]  = 'fa fa-inbox';
$tab['title'][] = $locale['vid_003'];
$tab['id'][]    = 'settings';
$tab['icon'][]  = 'fa fa-cogs';
...
switch ($_GET['section']) {
    ...
    if (dbcount("(video_cat_id)", DB_VIDEO_CATS)) {
            ...
            echo '<div class="well text-center">'.$locale['vid_004'].'</div>';
        }
        break;
    case 'categories':
